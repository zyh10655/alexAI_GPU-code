...
def main():
    ...
    total_tasks = len(sorted(glob.glob(os.path.join(folder, '*'))))
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        #Add the afterfix of SwinIR, extension changed
...
def setup(args):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        save_dir = f'SWINIR_results/swinir_{args.task}_x{args.scale}'
    ...
    # 004 grayscale image denoising/ 005 color image denoising
        ...
        save_dir = f'SWINIR_results/swinir_{args.task}_noise{args.noise}'
        folder = args.folder_gt
        border = 0
        window_size = 8
    ...
    # 006 JPEG compression artifact reduction
        ...
        save_dir = f'SWINIR_results/swinir_{args.task}_jpeg{args.jpeg}'
        folder = args.folder_gt
        border = 0
        window_size = 7
...
if __name__ == '__main__':
    main()
#Don't run the SWINIR for now (low memories)
#
# def run_script(task='gray_dn', scale=1, noise=50, jpeg=40, training_patch_size=128, large_model=False,
#                model_path='004_grayDN_DFWB_s128w8_SwinIR-M_noise50.pth', folder_lq=None,
#                folder_gt=None, tile=None, tile_overlap=32):
#     command = f"python ML\SWINIR\main_test_swinir.py"
#
#     if task:
#         command += f' --task {task}'
#     if scale:
...
#     if jpeg:
#         command += f' --jpeg {jpeg}'
#     if training_patch_size:
#         command += f' --training_patch_size {training_patch_size}'
#     if large_model:
#         command += ' --large_model'
#     if model_path:
...
#             break
#
#     print(f"Exit code: {process.returncode}")  # prints the exit code
